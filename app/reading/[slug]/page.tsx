...
type ExtractedTocItem = ReturnType<typeof extractTocFromMarkdown>[number];

function normalizeTocItem(item: ExtractedTocItem) {
const candidate = item as ExtractedTocItem & {
title?: string;
text?: string;
label?: string;
};
...
return {
...item,
title: candidate.title ?? candidate.text ?? candidate.label ?? "",
};
}
...
const toc = extractTocFromMarkdown(document.bodyMarkdown).map(normalizeTocItem);

return ( <article className="mx-auto max-w-3xl"> <header className="mb-10 border-b border-slate-200 pb-8"> <ReadingMetadata
