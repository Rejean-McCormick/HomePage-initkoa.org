...
**Destructive Action**  
An action that irreversibly modifies or deletes data (e.g., â€œDeleteâ€, â€œFormatâ€, â€œEraseâ€). Typically given a specific semantic pattern (e.g., `destructiveAction`) and intent (e.g., `DeleteItem`).
...
**Graph (UI Graph)**  
The representation of an applicationâ€™s UI as a directed graph, where nodes are states and edges are transitions. Stored in Atlas.
...
A possible, non-core consumer that uses Atlas to draw guidance on top of existing applications (highlights, arrows, step counters). Not part of Arianeâ€™s core specification.
...
- `Modal` â€“ blocking dialog.
- `primaryAction` â€“ main action in a state.
- `destructiveAction` â€“ action that deletes data.
...
Knowledge about *how to perform actions* or workflows (e.g., â€œhow to export a document as PDF in App Xâ€), as opposed to facts or static data. Ariane aims to represent procedural knowledge as UI graph data.
...
- [Background-UI-as-Data](../Background-UI-as-Data) â€“ conceptual motivation and knowledge domains.  
- [Theseus](../theseus) â€“ exploration engine and drivers.  
- [Atlas](../atlas) â€“ storage and semantic model.  
- [Consumers](../consumers) â€“ how external systems use Arianeâ€™s data.