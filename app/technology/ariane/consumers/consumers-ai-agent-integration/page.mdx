export const metadata = {
  title: "Consumers / AI Agent Integration"
};
...
From an agentâ€™s point of view, the interaction with Ariane typically follows this loop:
...
Ariane supports steps 2â€“4 by providing structured, semantic information about the UI.
...
- â€œExport this document as PDF.â€
- â€œChange the default font to Arial.â€
- â€œTurn on dark mode.â€
...
- Optional constraints or preferences (e.g., â€œuse the simplest pathâ€, â€œavoid destructive stepsâ€).

Ariane does not perform this mapping itself; it exposes a vocabulary of intents that the agent can align to.

See: [Atlas/Ontology-Vocabulary](../atlas-ontology-vocabulary)

---

## 2. State Recognition Against Atlas

To act correctly, the agent must know which state in Atlas corresponds to the userâ€™s current screen.
...
3. **Query Atlas**:
   - â€œGiven these fingerprint components and context (app, version, platform), what is the closest `state_id`?â€
