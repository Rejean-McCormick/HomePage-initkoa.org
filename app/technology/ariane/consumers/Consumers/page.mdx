...
This page describes the main categories of consumers and the typical ways they interact with Arianeâ€™s data.
...
- **AI agents**
  ...
  - Translate user goals (â€œexport this document as PDFâ€) into concrete UI paths.

- **Automation tools**
  - Use Atlas to generate or validate scripts that manipulate applications via their UI.
  - Prefer declarative workflows (â€œdo ExportToPDFâ€) over brittle, hard-coded sequences.
...
To interact meaningfully, a consumer first needs to know â€œwhere it isâ€ in the UI.
...
3. Query Atlas:
   - â€œGiven these fingerprints/hints, which state(s) are most similar?â€
...
- â€œWhat can be done from here?â€  
- â€œWhich actions correspond to a desired intent?â€
...
From Arianeâ€™s perspective, such a client:
...
See: [Consumers/Future-Overlay-Client](../consumers-future-overlay-client)

---

## Related Pages

- [Consumers/AI-Agent-Integration](../consumers-ai-agent-integration) â€“ more focused view on AI agents using Ariane.  
- [Atlas](../atlas) â€“ storage and semantic layer that consumers query.  
- [Atlas/Graph-Model](../atlas-graph-model) â€“ structural view of states and transitions.  
- [Atlas/core-Schema](../atlas-core-schema) â€“ fields available to consumers.  
- [Background-UI-as-Data](../background-ui-as-data) â€“ motivation for exposing UIs as data.