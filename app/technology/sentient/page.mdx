...
* pass candidates forward with traceable metadata.
...
| `status`            | `enum`   | Current lifecycle state, such as `NEW`, `PENDING`, `MATCHED`, `AMBIGUOUS`, `UNRESOLVED`, or `BLOCKED`. |                                     |
...
They may run before export or projection.
...
config/resolution/thresholds.json
config/resolution/authority_hints.json
config/resolution/property_boosts.json
config/export/projection_profiles.json
```
